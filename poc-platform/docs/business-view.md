...
- One web address and application catalog
...
3. **Refunds Dashboard** - supports refund review, approval thresholds, denial, and a simulated payout step.
...
- How the platform can start, monitor, and reset it

The platform checks these declarations before an application can run. For example, a prototype cannot claim to be production, use a production-stage classification, or quietly connect to an unsupported shared database.
...
A stakeholder opens one platform console and sees every available prototype as an application card. From there, the stakeholder can:
...
- Select a business persona
- Open the workflow through the same platform address
- Test how the experience changes by role
- Review health information and logs with the delivery team
- Reset the environment for a repeatable demonstration

The selected persona is mapped to the role that each application understands. For example, the same person can appear as a senior reviewer in the KYC application and a finance approver in the refunds application.
...
The platform enforces basic POC rules: synthetic or anonymized sample data, disposable local storage, simulated identity, and an explicit limitations list. A non-compliant prototype cannot be started from the console.
...
That hypothesis is **not yet proven**. The current applications use three different technology stacks and independently implement several common interface patterns. The next investment should therefore test reuse directly rather than assume the savings.
